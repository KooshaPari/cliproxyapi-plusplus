		...
		if result.Success {
			...
			if result.Model != "" {
				...
				case 408, 500, 502, 503, 504:
					if quotaCooldownDisabledForAuth(auth) || !m.hasAlternativeAuthForModelLocked(auth, result.Model, now) {
...
func (m *Manager) hasAlternativeAuthForModelLocked(auth *Auth, model string, now time.Time) bool {
	if m == nil || auth == nil || model == "" {
		return false
	}
	for _, candidate := range m.auths {
		...
		if !strings.EqualFold(candidate.Provider, auth.Provider) {
			continue
		}
