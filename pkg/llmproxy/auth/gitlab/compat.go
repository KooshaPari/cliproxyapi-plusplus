...
func TokenExpiry(now time.Time, token *TokenResponse) time.Time {
	...
	if token.CreatedAt > 0 && token.ExpiresIn > 0 {
		return time.Unix(token.CreatedAt+token.ExpiresIn, 0).UTC()
	}
...
type DirectAccessResponse struct {
	...
	ModelDetails *DirectAccessModelDetails `json:"model_details,omitempty"`
}
...
// GetCurrentUser fetches the authenticated user's profile from the GitLab API.
func (c *AuthClient) GetCurrentUser(ctx context.Context, baseURL, token string) (*User, error) {
	body, err := c.getJSON(ctx, http.MethodGet, NormalizeBaseURL(baseURL)+"/api/v4/user", token, "gitlab user")
	if err != nil {
		return nil, err
	}
	var user User
...
// GetPersonalAccessTokenSelf validates the supplied personal access token by
// querying the GitLab self-introspection endpoint.
func (c *AuthClient) GetPersonalAccessTokenSelf(ctx context.Context, baseURL, token string) (map[string]any, error) {
	body, err := c.getJSON(ctx, http.MethodGet, NormalizeBaseURL(baseURL)+"/api/v4/personal_access_tokens/self", token, "gitlab PAT self")
	if err != nil {
		return nil, err
	}
...
// FetchDirectAccess resolves the GitLab Duo code-suggestions gateway access
// details (base URL, gateway token, model metadata) for the given credential.
func (c *AuthClient) FetchDirectAccess(ctx context.Context, baseURL, token string) (*DirectAccessResponse, error) {
	body, err := c.getJSON(ctx, http.MethodPost, NormalizeBaseURL(baseURL)+"/api/v4/code_suggestions/direct_access", token, "gitlab direct access")
	if err != nil {
		return nil, err
	}
...
// getJSON performs an authenticated request against the GitLab API and returns
// the response body, surfacing non-2xx statuses as explicit errors.
func (c *AuthClient) getJSON(ctx context.Context, method, requestURL, token, scope string) ([]byte, error) {
	req, err := http.NewRequestWithContext(ctx, method, requestURL, nil)
	if err != nil {
		return nil, fmt.Errorf("%s request failed: %w", scope, err)
	}
	...
	body, err := io.ReadAll(resp.Body)
	if err != nil {
		return nil, fmt.Errorf("%s response read failed: %w", scope, err)
	}
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return nil, fmt.Errorf("%s request failed with status %d: %s", scope, resp.StatusCode, strings.TrimSpace(string(body)))
	}
	return body, nil
}
